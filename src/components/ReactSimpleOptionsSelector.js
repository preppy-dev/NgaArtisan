...
const Button = styled.input`
	background-color: ${props=> props.selected ? props.selected_background_color:"#fff"};
	color:  ${props=> props.selected ? props.selected_text_color:"#FF6201"};
	font:15px;
	font-weight:bold;
	height:40px;
	border:none;
	border: 1px solid ${props=> props.selected ? props.selected_border_color:"#FF6201"};
	cursor:pointer;
	padding: 0.25em 1em;
...
class ReactSimpleOptionsSelector extends React.Component{
  static PropTypes = {
	...
	optionClicked(event){
		...
		let selected = []
		this.props.options.forEach((option)=>{
			currentState[option.id] ? selected.push(option.value): null
		})
...
export default ReactSimpleOptionsSelector